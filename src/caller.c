...
static int call_not_bin(shell_t *shell, char **command_array,
    char **environment)
{
    char *command = command_array[0];
    int ret = execve(command, command_array, environment);
...
static int child_pid_actions(char **calls,
    char **command_array, shell_t *shell)
{
    char *command;
    int ret;
    ...
    for (int i = 0; i < my_strlen_array(calls); i++) {
        command = concat_str(3, calls[i], "/", command_array[0]);
        ret = execve(command, command_array, environment);
        free(command);
        if (ret != -1) {
            free_word_array(environment);
            exit(ret);
    ...
    call_not_bin(shell, command_array, environment);
    return 1;
}
...
void call_bin(char **command_array, shell_t *shell)
{
    ...
    child_pid = fork();
    if (child_pid == 0) {
        child_pid_actions(calls, command_array, shell);
    } else {
